...
int main(int argc, char* argv[]) { 
  ...
  */
  ...
  string leaves_array[MAX_LEAF_ADD];//store all leaf names
  ...
  if(!ifs2.is_open()) {
    ...
    while(ifs2.good()) {  
      ...
      if(token3 == "") {//only two tokens - FAMILY option
        taxa = token1;
        familyName = token2;
        
        ADDoption[leafcount] = FAMILY;
        cout<<"\nfamily ADD";
        int num = fam2id[familyName];
      ...
      cout<<"\nLeaf number "<<leafcount<<" anc1_id is: "<<anc1_id<<" anc2_id is: "<<anc2_id<<" lca "<<root;
      if(anc1_id == anc2_id) {
  ...
  for(unsigned int k=0; k<replicates; k++) {
	...
	if(selected_edge == t.root) {
	  cout<<"\nSelected Root ! continuing !";
	  continue;
	}
	else if ( selected_edge == subtree_root_node && ADDoption[j] == CROWN ) {
